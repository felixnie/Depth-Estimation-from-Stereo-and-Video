function pairwise = construct_graph_weighted(H, W, img)
...
% %
% % The graph describes a spatially varying smoothness cost. It's weighted
% % with the distance in color space between the adjacent pixels.
% %
% 
% % index matrix
% index = reshape(1:H*W, W, H)'; % row-wise indexing
...
% 
% % weights
% weights = abs(r(source) - r(target)) + ...
%           abs(g(source) - g(target)) + ...
%           abs(b(source) - b(target));
% % weights = ones(length(source), 1);
% pairwise = sparse(source, target, double(weights));